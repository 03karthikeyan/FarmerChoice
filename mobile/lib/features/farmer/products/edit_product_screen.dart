...
class EditProductScreen extends StatefulWidget {
  ...
  const EditProductScreen({super.key, required this.vegetable});
...
class _EditProductScreenState extends State<EditProductScreen> {
  ...
  void initState() {
    ...
    _tamilNameController = TextEditingController(text: widget.vegetable.tamilName);
    _priceController = TextEditingController(text: widget.vegetable.price.toString());
    _quantityController = TextEditingController(text: widget.vegetable.availableQuantity.toString());
    _descriptionController = TextEditingController(text: widget.vegetable.description);

    _category = _categories.contains(widget.vegetable.category) ? widget.vegetable.category : 'Vegetable';
    _priceUnit = _units.contains(widget.vegetable.priceUnit) ? widget.vegetable.priceUnit : 'kg';
    _availabilityStatus = widget.vegetable.availabilityStatus;
    if (widget.vegetable.images.isNotEmpty) {
      _uploadedImageUrl = widget.vegetable.images.first;
    }
  }
  ...
  void _submit() async {
    ...
    final ok = await farmerProv.updateProduct(
      id: widget.vegetable.id,
      name: name,
      ...
      images: _uploadedImageUrl != null ? [_uploadedImageUrl!] : widget.vegetable.images,
    );
    ...
    if (ok && mounted) {
      ...
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(
          content: Text('Vegetable updated successfully!'),
          backgroundColor: AppColors.primaryGreen,
        ),
      );
    } else if (mounted) {
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(content: Text('Failed to update vegetable listing.')),
      );
    }
  }
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      appBar: AppBar(
        title: const Text('Edit Vegetable Crop', style: TextStyle(fontWeight: FontWeight.w800, color: AppColors.textDark)),
        backgroundColor: Colors.white,
        elevation: 0,
        leading: IconButton(
      ...
      body: SingleChildScrollView(
        ...
        child: Column(
          ...
          children: [
            ...
            const Text(
              'Update crop price, available stock, or replace the photo.',
                    ...
                    : Stack(
                        children: [
                          if (_uploadedImageUrl == null)
                            const Center(
                              child: Icon(Icons.add_a_photo_rounded, size: 48, color: Color(0xFF176B2C)),
                            ),
                          Positioned(
                            top: 10,
                            right: 10,
